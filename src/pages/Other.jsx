...
import {
  ...
  StatusBadge, EmptyState, Dialog, FormField, Input, Select, Textarea, toast, PillTabs
} from '../components/ui'
import { ChevronLeft, ChevronRight, Plus, ArrowRight, Edit2, Trash2 } from 'lucide-react'
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold tracking-tight">{monthNames[m]} {y}</h2>
          <p className="text-sm text-muted-foreground">Měsíční přehled</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex bg-muted rounded-xl p-1 gap-0.5">
            {['month','week'].map(v => (
              <button key={v} onClick={() => setView(v)} className={cn('px-4 py-1.5 rounded-lg text-sm font-medium transition-all', view===v ? 'bg-white shadow-sm font-semibold' : 'text-muted-foreground hover:text-foreground')}>
                {v === 'month' ? 'Měsíc' : 'Týden'}
              </button>
            ))}
          </div>
      ...
      <Card className="overflow-hidden">
        <div className="grid grid-cols-7 border-b border-border">
          {dayNames.map(d => <div key={d} className="py-3 text-center text-[11px] font-bold uppercase tracking-wider text-muted-foreground bg-muted/30">{d}</div>)}
        </div>
        <div className="grid grid-cols-7">
          {Array.from({length:startDay}).map((_,i) => <div key={'e'+i} className="min-h-[72px] sm:min-h-[90px] border-r border-b border-border bg-muted/20"/>)}
          {Array.from({length:lastDay.getDate()}).map((_,i) => {
            ...
            return (
              <div key={d} className={cn('min-h-[72px] sm:min-h-[90px] border-r border-b border-border p-1.5 cursor-pointer transition-colors hover:bg-green-25', isToday && 'bg-green-50')}>
                <div className={cn('w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold mb-1', isToday ? 'bg-primary text-white' : '')}>{d}</div>
                {dos.slice(0,2).map(o => {
                  const c = getClient(o.clientId)
                  return <div key={o.id} className="text-[10px] font-semibold bg-green-100 text-green-800 px-1 py-0.5 rounded truncate mb-0.5">{c?.name?.split(' ')[0]}</div>
                })}
      ...
      <div>
        ...
        {upcoming.length === 0 ? <p className="text-sm text-muted-foreground">Žádné naplánované zakázky</p> : (
              ...
              return (
                <Card key={o.id}>
                  <CardContent className="p-4 flex items-center gap-4 flex-wrap sm:flex-nowrap">
                    <div className="text-center w-14 bg-green-50 border border-green-100 rounded-xl py-2 flex-shrink-0">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h2 className="text-lg font-bold tracking-tight">Ceník služeb</h2><p className="text-sm text-muted-foreground">Ceny pro automatické kalkulace</p></div>
        ...
        {services.map(s => (
          <Card key={s.id}>
            <CardContent className="p-4 flex items-center gap-4">
              <div className="flex-1"><p className="font-semibold">{s.name}</p><p className="text-xs text-muted-foreground">za {s.unit}</p></div>
              <p className="font-bold text-xl tracking-tight text-primary">{s.pricePerUnit} Kč</p>
              <div className="flex gap-1.5">
                <Button size="icon-sm" onClick={() => openEdit(s)}><Edit2 size={12}/></Button>
                <Button variant="danger" size="icon-sm" onClick={() => { deleteService(s.id); toast('Odstraněno','warning') }}><Trash2 size={12}/></Button>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Upravit cenu' : 'Nová služba'}
        ...
        <FormField label="Název služby *">
          <Input value={form.name} onChange={e => setForm(f=>({...f,name:e.target.value}))} placeholder="Sekání trávy" />
        </FormField>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-6">
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="ghost" onClick={() => toast('Vše označeno přečteno')}>Označit vše přečteno</Button>
        </div>
        <Card>
          {notifs.map(n => (
            <div key={n.id} className={cn('flex items-start gap-3 px-5 py-4 border-b border-border last:border-0', !n.read && 'bg-green-50/40')}>
              <span className={cn('w-2 h-2 rounded-full mt-2 flex-shrink-0', dotColor[n.type])}/>
              <div className="flex-1"><p className={cn('text-sm', !n.read&&'font-medium')}>{n.message}</p><p className="text-xs text-muted-foreground mt-0.5">{n.time} zpět</p></div>
              {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-primary mt-2 flex-shrink-0"/>}
            </div>
          ))}
        </Card>

        <div>
          <h3 className="text-sm font-semibold mb-3">Klienti bez kontaktu (21+ dní)</h3>
          <div className="space-y-2">
            {clients.filter(c=>c.status==='active').slice(0,4).map(c => {
              ...
              const needs = days > 21
              return (
                <Card key={c.id} className={needs?'border-amber-200':''}>
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate">{c.name}</p><p className={cn('text-xs', needs?'text-amber-600 font-medium':'text-muted-foreground')}>{last?`Naposledy: před ${days} dny`:'Žádná zakázka'}{needs?' — čas na připomenutí':''}</p></div>
                    <Button variant={needs?'primary':'secondary'} size="sm" onClick={() => toast(`SMS odesláno na ${c.phone}`)}>SMS</Button>
                  </CardContent>
                </Card>
              )
            })}
      ...
      <div className="space-y-5">
        <Card>
          ...
          <CardContent className="p-4 space-y-2">
            {[['Připomenutí návštěvy','Den předem'],['Zakázka dokončena','Poděkování'],['Faktura k úhradě','Upomínka'],['Vlastní zpráva','Vlastní text']].map(([t,d])=>(
              <button key={t} onClick={() => toast(`Šablona "${t}" - funkce SMS brzy`,'info')} className="w-full flex items-center justify-between gap-3 p-3 rounded-xl border border-border hover:border-primary hover:bg-green-50 transition-all text-left">
          ...
          <CardContent className="p-4 space-y-2">
            {seasons.map(s => (
              <div key={s.label} className={cn('flex items-center gap-3 p-3 rounded-xl border', s.style)}>
                ...
                <Button size="sm" onClick={() => toast(`Kampaň "${s.label}" odeslána`, 'success')}>Rozeslat</Button>
              </div>
            ))}
          </CardContent>
        </Card>
...
export function Settings() {
  ...
  const toggles = [['Připomenutí den před zakázkou','SMS klientovi automaticky',true],['Potvrzení dokončené zakázky','SMS po dokončení',true],['Upomínka nezaplacené faktury','Email po 7 dnech',true],['Sezónní kampaně','Automatické nabídky na sezónu',false]]

  function exportData() {
    const blob = new Blob([JSON.stringify({clients,orders,invoices,date:new Date().toISOString()},null,2)],{type:'application/json'})
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      <div className="flex bg-muted rounded-xl p-1 gap-0.5 w-fit">
        {tabs.map(t => <button key={t.value} onClick={()=>setTab(t.value)} className={cn('px-5 py-2 rounded-lg text-sm font-medium transition-all', tab===t.value?'bg-white shadow-sm font-semibold':'text-muted-foreground hover:text-foreground')}>{t.label}</button>)}
      </div>
      ...
      {tab==='data' && (
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Přehled dat</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-3 gap-4">
              {[['Klientů',clients.length],['Zakázek',orders.length],['Faktur',invoices.length]].map(([l,v])=>(
                <div key={l} className="bg-muted rounded-xl p-4 text-center"><p className="text-2xl font-bold tracking-tight">{v}</p><p className="text-xs text-muted-foreground uppercase tracking-wider mt-1">{l}</p></div>
              ))}
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent className="space-y-3">
              {[['Export dat (JSON)','Záloha všech dat',exportData],['Export faktur (CSV)','Pro POHODA, Money S3',()=>toast('CSV exportováno')]].map(([t,d,fn])=>(
                <div key={t} className="flex items-center justify-between p-4 bg-muted/50 rounded-xl">
                  <div><p className="text-sm font-semibold">{t}</p><p className="text-xs text-muted-foreground">{d}</p></div>
                  <Button size="sm" onClick={fn}>Export</Button>
                </div>
              ))}
            </CardContent>
          </Card>
          <Card className="border-destructive/20 bg-red-50/30">
            ...
            <CardContent>
              <div className="flex items-center justify-between p-4 bg-white rounded-xl border border-red-200">
                ...
                <Button variant="danger" size="sm" onClick={() => { if(confirm('Opravdu smazat vše?')) { ['zp3_clients','zp3_orders','zp3_invoices','zp3_services'].forEach(k=>localStorage.removeItem(k)); toast('Data smazána','warning'); setTimeout(()=>window.location.reload(),1000) } }}>Smazat</Button>
              </div>
            </CardContent>
          </Card>
        </div>
